...
class Room {
  static async create(propertyId, roomNumber, roomType, capacity, monthlyRent, cautionDeposit, isAvailable, description, images) {
    const [result] = await pool.execute(
      `INSERT INTO rooms (propertyId, roomNumber, roomType, capacity, monthlyRent,
                          cautionDeposit, isAvailable, description, images)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [propertyId, roomNumber, roomType, capacity, monthlyRent,
       cautionDeposit ?? null, isAvailable ?? true,
       description ?? null,
  ...
  static async findByPropertyId(propertyId) {
    const [rows] = await pool.execute(
      "SELECT * FROM rooms WHERE propertyId = ? ORDER BY roomNumber ASC", [propertyId]
    );
    return rows.map((r) => ({
      id: r.id, _id: String(r.id),
      propertyId: r.propertyId,
      roomNumber: r.roomNumber,
      type: r.roomType, roomType: r.roomType,
      capacity: r.capacity,
  ...
  static async update(id, { roomNumber, roomType, capacity, monthlyRent, cautionDeposit, isAvailable, description, images }) {
    const [result] = await pool.execute(
      `UPDATE rooms SET roomNumber=?, roomType=?, capacity=?, monthlyRent=?,
                        cautionDeposit=?, isAvailable=?, description=?, images=?
       WHERE id=?`,
      [roomNumber, roomType, capacity, monthlyRent,
       cautionDeposit ?? null, isAvailable ?? true,
       description ?? null,
       JSON.stringify(Array.isArray(images) ? images : []),
       id]
  ...
  static async updateAvailability(id, isAvailable) {
    const [result] = await pool.execute(
      "UPDATE rooms SET isAvailable = ? WHERE id = ?", [isAvailable, id]
    );
    return result.affectedRows;
  }
}
