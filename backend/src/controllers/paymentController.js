...
exports.getPaymentsByLandlord = async (req, res, next) => {
  try {
    const payments = await PaymentService.getPaymentsByLandlordId(req.user.id);
    res.status(200).json(payments);
  } catch (error) { next(error); }
};

// NEW: tenant sees payments linked to their bookings
exports.getPaymentsByGuest = async (req, res, next) => {
  try {
    const payments = await PaymentService.getPaymentsByGuestId(req.user.id);
    res.status(200).json(payments);
  } catch (error) { next(error); }
};
...
exports.updatePayment = async (req, res, next) => {
  try {
    const affectedRows = await PaymentService.updatePayment(req.params.id, req.body);
    if (affectedRows === 0) return res.status(404).json({ message: "Payment not found or no changes made" });
...
exports.deletePayment = async (req, res, next) => {
  try {
    const affectedRows = await PaymentService.deletePayment(req.params.id);
